...
class iCalUtilityFunctions
{
    ...
    public static function createTimezone(& $calendar, $timezone, $xProp = array())
    {
        ...
        if (($stdTZOFFSETTO != $stdTZOFFSETFROM) && (false === iCalUtilityFunctions::_setTZrrule($std))) {
            $std->setProperty('RRULE',
                array('FREQ' => 'YEARLY', 'BYDAY' => array('-1', 'DAY' => 'SU'), 'BYMONTH' => 10));
        }
        ...
        if (false === iCalUtilityFunctions::_setTZrrule($dlght)) {
            $dlght->setProperty('RRULE',
                array('FREQ' => 'YEARLY', 'BYDAY' => array('-1', 'DAY' => 'SU'), 'BYMONTH' => 3));
        }
    ...
    public static function _date2timestamp($datetime, $tz = null)
    {
        ...
        $output = mktime($datetime['hour'], $datetime['min'], ($datetime['sec'] + $offset), $datetime['month'],
    ...
    public static function _duration2date($startdate = null, $duration = null)
    {
        ...
        $dtend = mktime($startdate['hour'], $startdate['min'], ($startdate['sec'] + $dtend), $startdate['month'],
    ...
    public static function _format_date_time($datetime, $parno = 6)
    {
        ...
        if (isset($datetime['hour']) ||
            ...
            $date = mktime($datetime['hour'], $datetime['min'], $datetime['sec'], $datetime['month'], $datetime['day'],
                $datetime['year']);
            ...
            if (isset($datetime['tz']) && ('' < trim($datetime['tz']))) {
                ...
                if (0 != $offset) {
                    $date = mktime($datetime['hour'], $datetime['min'], ($datetime['sec'] + $offset),
    ...
    public static function _isArrayDate($input)
    {
        ...
        $input = iCalUtilityFunctions::_date_time_string($input[1] . '/' . $input[2] . '/' . $input[0],
            3); //  m - d - Y
        if (isset($input['year']) && isset($input['month']) && isset($input['day'])) {
            return checkdate((int)$input['month'], (int)$input['day'], (int)$input['year']);
        }
...
// echo "bysetposXold_start=$bysetposYold $bysetposMold $bysetposDold<br />\n"; // test ###
            ...
            if ('YEARLY' == $recur['FREQ']) {
                ...
                $wdatets = iCalUtilityFunctions::_date2timestamp($wdate);
                iCalUtilityFunctions::_stepdate($enddate, $endDatets,
                    array('year' => 1)); // make sure to count whole last year
            } elseif ('MONTHLY' == $recur['FREQ']) {
                $wdate['day'] = 1; // start from beginning of month
                $wdatets = iCalUtilityFunctions::_date2timestamp($wdate);
                iCalUtilityFunctions::_stepdate($enddate, $endDatets,
                    array('month' => 1)); // make sure to count whole last month
            } else {
...
// echo "<br />\n"; // test ###
    ...
    public static function _setDate(
        ...
        $input = $parno = null;
        $localtime = (('dtstart' == $caller) && in_array($objName,
                array('vtimezone', 'standard', 'daylight'))) ? true : false;
    ...
    public static function _setTZrrule(& $obj)
    {
        ...
        $ts = mktime((int)$date['hour'], (int)$date['min'], (int)$date['sec'], (int)$date['month'], (int)$date['day'],
            (int)$date['year']);
        ...
        $obj->setProperty('RRULE', array(
            'FREQ' => 'YEARLY',
            'BYDAY' => array((string)$ordwk, 'DAY' => $day),
            'BYMONTH' => (int)$date['month']
        ));
