...
// if( isset($end))  echo 'selectComp 5 start='.implode('-',$start).' end='.implode('-',$end)."<br />\n"; // test ###
            ...
            while (false !== ($exdate = $component->getProperty('exdate'))) {  // check exdate
                foreach ($exdate as $theExdate) {
                    ...
                    $exWdate = mktime(0, 0, 0, date('m', $exWdate), date('d', $exWdate),
                        date('Y', $exWdate)); // on a day-basis !!!
                    ...
                    foreach ($rdate as $theRdate) {
                        if (is_array($theRdate) && (2 == count($theRdate)) &&  // all days within PERIOD
                            ...
                            while ($rstart < $rend) {
                                ...
                                $rstart = mktime(date('H', $rstart), date('i', $rstart), date('s', $rstart),
                                    date('m', $rstart), date('d', $rstart) + 1, date('Y', $rstart)); // step one day
                            }
                        } // PERIOD end
                        else { // single date
...
// echo "recurKey=".date( 'Y-m-d H:i:s', $recurkey ).' dur='.iCalUtilityFunctions::offsetSec2His( $durvalue )."<br />\n"; // test ###;
                        ...
                        $checkDate = mktime(0, 0, 0, date('m', $recurkey), date('d', $recurkey),
                            date('Y', $recurkey)); // on a day-basis !!!
...
// echo "splitStart=".date( 'Y-m-d H:i:s', $rstart ).' end='.date( 'Y-m-d H:i:s', $rend )."<br />\n"; // test ###;
                            while ($rstart <= $rend) { // iterate.. .
                                $checkDate = mktime(0, 0, 0, date('m', $rstart), date('d', $rstart),
                                    date('Y', $rstart)); // on a day-basis !!!
...
//echo "X-CURRENT-DTSTART 1 = $datestring xRecurrence=$xRecurrence tcnt =".++$tcnt."<br />";$component2->setProperty( 'X-CNT', $tcnt ); // test ###
                                    ...
                                    if ($dtendExist || $dueExist || $durationExist) {
                                        ...
                                        {
                                            $tend = mktime(23, 59, 59, date('m', $rstart), date('d', $rstart),
                                                date('Y', $rstart));
                                        } else {
                                            $tend = mktime(date('H', $endWdate), date('i', $endWdate),
                                                date('s', $endWdate), date('m', $rstart), date('d', $rstart),
                                                date('Y', $rstart));
                                        } // on a day-basis !!!
...
//echo "X-CURRENT-DTSTART 2 = $datestring xRecurrence=$xRecurrence tcnt =".++$tcnt."<br />";$component2->setProperty( 'X-CNT', $tcnt ); // test ###
                                ...
                                if ($dtendExist || $dueExist || $durationExist) {
                                    ...
                                    if (date('Ymd', $rstart) < date('Ymd', $endWdate)) {
                                        ...
                                        $tend = mktime(date('H', $endWdate), date('i', $endWdate), date('s', $endWdate),
                                            date('m', $rstart), date('d', $rstart), date('Y', $rstart));
                                    } // on a day-basis !!!
                                    $datestring = date($endDateFormat, $tend);
                                    if (isset($end['tz'])) {
                                        $datestring .= ' ' . $end['tz'];
                ...
                if ( ! isset($exdatelist[$checkDate])) { // exclude any recurrence START date, found in exdatelist
                    ...
                    while ($rstart <= $endWdate) { // iterate
                        if ($rstart > $startWdate) { // if NOT startdate, set X-properties
                            $datestring = date($startDateFormat,
                                mktime(0, 0, 0, date('m', $rstart), date('d', $rstart), date('Y', $rstart)));
                            if (isset($start['tz'])) {
                                $datestring .= ' ' . $start['tz'];
                            }
// echo "X-CURRENT-DTSTART 3 = $datestring xRecurrence=$xRecurrence tcnt =".++$tcnt."<br />";$component->setProperty( 'X-CNT', $tcnt ); // test ###
                        ...
                        $rstart = mktime(date('H', $rstart), date('i', $rstart), date('s', $rstart), date('m', $rstart),
                            date('d', $rstart) + 1, date('Y', $rstart)); // step one day
                    } // end while( $rstart <= $endWdate )
                } // end if( !isset( $exdatelist[$checkDate] ))
            } // end if( $split )   -  else use component date
            elseif ($startWdate >= $startDate) {          // within period
                $checkDate = mktime(0, 0, 0, date('m', $startWdate), date('d', $startWdate),
                    date('Y', $startWdate)); // on a day-basis !!!
                if ( ! isset($exdatelist[$checkDate])) { // exclude any recurrence START date, found in exdatelist
                    foreach (array(
                                 'X-CURRENT-DTSTART',
                                 'X-CURRENT-DTEND',
    ...
    function setComponent($component, $arg1 = false, $arg2 = false)
    {
        ...
        if (ctype_digit((string)$arg1)) { // index insert/replace
            $argType = 'INDEX';
            $index = (int)$arg1 - 1;
        } elseif (in_array(strtolower($arg1),
        ...
        if (isset($this->unparsed) && is_array($this->unparsed) && (0 < count($this->unparsed))) {
            ...
            foreach ($proprows as $line) {
                ...
                for ($cix = 0; $cix < $strlen; $cix++) {
                    if ((':' == $line[$cix]) &&
                        ('://' != substr($line, $cix, 3)) &&
                        ( ! in_array(strtolower(substr($line, $cix - 3, 4)),
                            array('fax:', 'cid:', 'sms:', 'tel:', 'urn:'))) &&
...
class calendarComponent
{
            ...
            foreach ($attendeePart as $paramlabel => $paramvalue) {         // start foreach 2
                if ('value' == $paramlabel) {
                    ...
                    foreach ($paramvalue as $optparamlabel => $optparamvalue) { // start foreach 3
                        ...
                        if ( ! in_array($optparamlabel, array(
                            'CUTYPE',
                            'MEMBER',
                            'ROLE',
                            'PARTSTAT',
    ...
    function setCreated(
        ...
        if ( ! isset($year)) {
            $year = date('Ymd\THis',
                mktime(date('H'), date('i'), date('s') - date('Z'), date('m'), date('d'), date('Y')));
        }
    ...
    function createDuration()
    {
        ...
        return $this->_createElement('DURATION', $attributes,
            iCalUtilityFunctions::_format_duration($this->duration['value']));
    }
    ...
    function setDuration($week, $day = false, $hour = false, $min = false, $sec = false, $params = false)
    {
        ...
        if (is_array($week) && (1 <= count($week))) {
            $this->duration = array(
                'value' => iCalUtilityFunctions::_duration_array($week),
            ...
            $this->duration = array(
                'value' => iCalUtilityFunctions::_duration_string($week),
                'params' => iCalUtilityFunctions::_setParams($day)
            );
            ...
            $this->duration = array(
                'value' => iCalUtilityFunctions::_duration_array(array(
                    $week,
                    $day,
                    $hour,
                    $min,
                ...
                if (isset($input['params']['VALUE']) && ('PERIOD' == $input['params']['VALUE'])) { // PERIOD
                    foreach ($theRdate as $rix => $rPeriod) {
                        if (is_array($rPeriod)) {
                            ...
                            {
                                $inputab = (isset($rPeriod['tz'])) ? iCalUtilityFunctions::_timestamp2date($rPeriod,
                                    $parno) : iCalUtilityFunctions::_timestamp2date($rPeriod, 6);
                            } elseif (iCalUtilityFunctions::_isArrayDate($rPeriod)) {
                                $inputab = (3 < count($rPeriod)) ? iCalUtilityFunctions::_date_time_array($rPeriod,
                                    $parno) : iCalUtilityFunctions::_date_time_array($rPeriod, 6);
