        ...
        foreach ($lines as $line) {
            ...
            switch ($type) {
                case '=':
                    continue;
                case '+':
                    $ret .= "\t" . '<li class="THM-iCampus-added"><span></span>' . htmlentities(trim(substr($line,
                            2))) . "</li>\n";
                    break;

                case '-':
                    $ret .= "\t" . '<li class="THM-iCampus-removed"><span></span>' . htmlentities(trim(substr($line,
                            2))) . "</li>\n";
                    break;

                case '~':
                    $ret .= "\t" . '<li class="THM-iCampus-changed"><span></span>' . htmlentities(trim(substr($line,
                            2))) . "</li>\n";
                    break;

                case '!':
                    $ret .= "\t" . '<li class="THM-iCampus-important"><span></span>' . htmlentities(trim(substr($line,
                            2))) . "</li>\n";
                    break;

                case '#':
                    $ret .= "\t" . '<li class="THM-iCampus-fixed"><span></span>' . htmlentities(trim(substr($line,
                            2))) . "</li>\n";
                    break;
