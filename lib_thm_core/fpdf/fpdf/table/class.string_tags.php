...
class String_TAGS
{
	...
	function OpenTag($p_tag, $p_array)
	{
		...
		if (!@eregi("^<([a-zA-Z1-9]{1,$maxElem}) *[href=]*(.*)>$", $p_tag, $reg))
		{
			return false;
		}
		...
		if (strlen($sHREF) > 0)
		{
			...
			if (is_int(strpos("\"\'", $sHREF[0])))
			{
				$sHREF = substr($sHREF, 1, strlen($sHREF));
			}
			if (is_int(strpos("\"\'", $sHREF[strlen($sHREF) - 1])))
			{
				$sHREF = substr($sHREF, 0, strlen($sHREF) - 1);
			}
		}
	...
	function CloseTag($p_tag, $p_array)
	{
		...
		if (!@ereg("^</([a-zA-Z1-9]{1,$maxElem})>$", $p_tag, $reg))
		{
			return false;
		}
