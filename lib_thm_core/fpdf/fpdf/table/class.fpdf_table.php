...
//Draws the Header
	...
	function DrawTableLine($data, $header, $pDataType = 0)
	{
		...
		for ($i = 0; $i < $this->tb_columns; $i++)
		{
			...
			{
				$data[$i]['TEXT_STRLINES'] = $this->StringToLines($data[$i]['CELL_WIDTH'], utf8_decode($data[$i]['TEXT']));
			}
		...
		{//data line
			...
			if (!$this->tb_split_normal)
			{//split only if the cell height is bigger than a page size

				if ($split && (($hm + $this->tb_header_height) < $MaxPageH))
				{//if the header is splitted and it has space on a page
