    ...
    function End_Page_Border()
    {
        if (isset($this->tb_table_type['BRD_TYPE_END_PAGE'])) {
            ...
            if (strpos($this->tb_table_type['BRD_TYPE_END_PAGE'], 'B') >= 0) {
                ...
                //draw the line
                $this->Line($this->table_startx, $this->GetY(), $this->table_startx + $this->Get_Table_Width(),
                    $this->GetY());
            }
        }
    }
    ...
    function DrawTableLine($data, $header, $pDataType = 0)
    {
        ...
        for ($i = 0; $i < $this->tb_columns; $i++) {
            ...
            if ( ! isset($data[$i]['TEXT_STRLINES'])) {
                $data[$i]['TEXT_STRLINES'] = $this->StringToLines($data[$i]['CELL_WIDTH'],
                    utf8_decode($data[$i]['TEXT']));
            }
