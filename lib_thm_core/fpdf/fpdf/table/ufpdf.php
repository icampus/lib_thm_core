        ...
        function Cell($w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = 0, $link = '')
        {
            ...
            if ($txt != '') {
                ...
                $s .= sprintf('BT %.2f %.2f Td %s Tj ET', ($this->x + $dx) * $k,
                    ($this->h - ($this->y + .5 * $h + .3 * $this->FontSize)) * $k, $txtstring);
                if ($this->underline) {
                    $s .= ' ' . $this->_dounderline($this->x + $dx, $this->y + .5 * $h + .3 * $this->FontSize, $width,
                            $txt);
                }
                if ($this->ColorFlag) {
                    $s .= ' Q';
                ...
                if ($link) {
                    $this->Link($this->x + $dx, $this->y + .5 * $h - .5 * $this->FontSize, $width, $this->FontSize,
                        $link);
                }
            }
            if ($s) {
